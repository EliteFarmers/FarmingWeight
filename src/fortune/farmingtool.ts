...
export class FarmingTool extends UpgradeableBase {
  ...
  public override get type(): ReforgeTarget {
    ...
    if (Object.values(ReforgeTarget).includes(t as ReforgeTarget))
      return t as ReforgeTarget;
  ...
  getUpgrades(): FortuneUpgrade[] {
    ...
    const upgrades = getSourceProgress<FarmingTool>(
      this,
      TOOL_FORTUNE_SOURCES,
      false
  ...
  rebuildTool(item: EliteItemDto, options?: PlayerOptions) {
    ...
    if (!tool) {
      throw new Error(
        `Unknown farming tool: ${item.name} (${item.skyblockId})`
      );
    }
    ...
    this.farmingForDummies = +(
      this.item.attributes?.farming_for_dummies_count ?? 0
    );
  ...
  getFortune(): number {
    ...
    const baseRarity = this.recombobulated
      ? previousRarity(this.rarity)
      : this.rarity;
    const rarityStats = getStatValue(
      this.tool.stats?.[baseRarity]?.[Stat.FarmingFortune]
    );
    ...
    for (const [enchant, level] of enchantments) {
      ...
      if (enchantment.cropSpecific && enchantment.cropSpecific !== this.crop)
        continue;

      const fortune = getFortuneFromEnchant(
        level,
        enchantment,
        this.options,
        this.crop
  ...
  supportsCultivating(): boolean {
    return this.type
      ? FARMING_ENCHANTS.cultivating?.appliesTo.includes(this.type) ?? false
      : false;
  }
  ...
  isMissingDedication() {
    ...
    return (
      ...
      (this.options?.milestones?.[this.crop] ?? 0) <= 0
    );
  }
  ...
  hasAxedPerk(): boolean {
    ...
    const cropFound =
      getNumberFromMatchingLine(this.item.lore ?? [], cropRegex) ?? 0;
  ...
  static isValid(item: EliteItemDto | FarmingTool): boolean {
    ...
    return (
      FARMING_TOOLS[item.skyblockId as keyof typeof FARMING_TOOLS] !== undefined
    );
  }
